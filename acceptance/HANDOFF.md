...
> `acceptance/README.md` (blueprint), `TASKS.md` § "Superapp", y las
...
Arnés **headless** en `acceptance/` que ejerce TODA la superficie pública
...
2. **Headless**, dentro de `acceptance/`, sin framework web y **sin deps
...
6. **Slices compilables.** Cada paso termina compilando y corriendo al menos en
   ...
   el primer `go build ./acceptance/...` (Go 1.25.7) es tuyo — corrige
...
- **Verificación de infra (observabilidad + caché) — `recorder/infra_test.go`,
  ...
  `go test -tags=superapp_infra -run TestObservabilityAndCacheInfra ./acceptance/recorder/`.
...
**S6 · `main.go` — ✅ HECHO.** `acceptance/main.go` (root, `package main`)
