...
func init() {
	...
	quark.RegisterGeneratedMeta(reflect.TypeOf(Doc{}), quark.GeneratedMeta{
		ContractVersion: 3,
		ModelHash:       "f68ef7bc6dbab9602e559049e505689348a4bc92f6c8752c7e24ac59eeb0380c",
	})
	quark.RegisterTypedScanner(reflect.TypeOf(Doc{}), quarkgenScanDoc)
	quark.RegisterTypedBinder(reflect.TypeOf(Doc{}), quark.StubBinder)
}
...
func quarkgenScanDoc(rows *sql.Rows, dest any) error {
	m := dest.(*Doc)
	cols, err := rows.Columns()
	if err != nil {
		return err
	...
	for i, c := range cols {
		switch strings.ToLower(c) {
		case "code":
			scanDest[i] = quark.ScanTarget(&m.Code)
		case "title":
			scanDest[i] = quark.ScanTarget(&m.Title)
		default:
			var discard any
			scanDest[i] = &discard
		}
...
// DocColumns provides typed, compile-time-checked column accessors for Doc.
var DocColumns = struct {
	Code  quark.TypedStringColumn
	Title quark.TypedStringColumn
}{
	Code:  quark.NewTypedStringColumn("code"),
	Title: quark.NewTypedStringColumn("title"),
}
