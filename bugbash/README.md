...
| Fases F0-F14 | 🚧 Hechas: **F0** (`f00_install`), **F1** (`f01_smoke`), **F2** (`f02_api_surface`), **F3** (`f03_relaciones`), **F4** (`f04_volume`, cap/paginación/streaming/chunking — halló y cerró BB-10), **F5** (`f05_tenancy`), **F7** (`f07_cache`), **F8** (`f08_hooks`), **F13** (`f13_security`). Pendientes: F6, F9, F10, F11, F12, F14 (una por sesión). Leg de codegen de F0 (`quark gen`) pendiente. Nota: F13 está implementada fuera de orden por ser el gate obligatorio antes de cualquier patch v1.0.x. |
