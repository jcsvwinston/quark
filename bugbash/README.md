...
| Fases F0-F14 | ✅ **COMPLETO (F0-F14)**: **F0** (`f00_install`), **F1** (`f01_smoke`), **F2** (`f02_api_surface`), **F3** (`f03_relaciones`), **F4** (`f04_volume` — BB-10), **F5** (`f05_tenancy`), **F6** (`f06_migrations` — BB-11, BB-12), **F7** (`f07_cache`), **F8** (`f08_hooks`), **F9** (`f09_codegen`), **F10** (`f10_sharding`), **F11** (`f11_replicas`), **F12** (`f12_resilience`), **F13** (`f13_security`), **F14** (`f14_soak`, time-boxed — sin hallazgos; la pasada RC de 12h × 6 motores queda para ventana release-candidate). Bugs cerrados por el bug-bash: BB-1…BB-12. Leg de codegen de F0 (`quark gen`) pendiente. Nota: F13 fuera de orden por ser el gate obligatorio antes de patches v1.0.x. |
