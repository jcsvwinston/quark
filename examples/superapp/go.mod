// The acceptance harness is its own module so that what it needs to exercise
// every engine — the five driver modules and their engine libraries — stays
// out of the library's go.mod (ADR-0024). It is never published: the local
// replace directives point every Quark requirement back into this tree, the
// way the runnable examples do.
module github.com/jcsvwinston/quark/examples/superapp
