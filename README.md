...
The **v1.1.1–v1.1.4** patches carry cross-engine correctness fixes — `CreateBatch` primary-key back-fill on Oracle/MySQL/SQL Server, recursive CTEs and set-op pagination on Oracle/SQL Server, and `BeforeCreate`/`BeforeUpdate` hooks on the batch and upsert paths — detailed in the [`CHANGELOG`](CHANGELOG.md). All non-breaking.
