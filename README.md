...
Quark is **v1.13.0** <!-- x-release-please-version --> on the stable `v1.x` line
(v1.2.0 closed the scaling deferrals; v1.1.0 was the hardening minor; v1.0.0 the first stable release under SemVer). Phases 0–6 are complete: the core query builder, CRUD, schema-as-code migrations across all six dialects (Oracle now in blocking CI), multi-tenancy, caché, hooks/events/audit log, observability, opt-in code generation, read replicas with failover, and pluggable sharding. `v1.x` keeps API compatibility; breaking changes go to `v2.x` with a `docs/MIGRATION_v2.0.0.md`. Symbols on their way out are listed in [`docs/deprecations/`](docs/deprecations/), each with its replacement, the release it is removed in and the date that window opens. v1.0 was gated on the qualitative checklist in [`docs/V1_GATE.md`](docs/V1_GATE.md) (cross-engine coverage, structural gaps closed or consciously waived), not on a performance target — [ADR-0017](docs/adr/0017-codegen-type-safety-not-perf-gate.md) retired the ≥3× codegen performance gate, so code generation is a type-safety feature, not a speedup. Known limitations consciously deferred at each minor are listed in the release notes ([`docs/RELEASE_NOTES_v1.13.0.md`](docs/RELEASE_NOTES_v1.13.0.md) for the current line).

The **v1.10.0** minor makes the missing-driver error name the module to import. The previous **v1.9.0** minor takes the database drivers out of the library: each engine
ships as its own module, so a program links only the one it uses (24 MB → 6 MB,
171 modules → 129). Add one blank import — `drivers/postgres`, `drivers/mysql`,
`drivers/sqlite`, `drivers/mssql` or `drivers/oracle` — and nothing else
changes. The previous **v1.8.0** minor closes the core and CLI findings of an end-to-end audit,
...
**What `go.mod` lists is not what a binary links.** The root module still
*requires* the five engine drivers and the test-container modules: `cmd/quark`
is a tool that talks to whatever database it is pointed at, so the CLI links
every engine, and the engine suites run from this module. A program that
imports `quark` carries 128 modules in its build list and links 11 of them; the
117 it never links are not in the binary, and `go build` does not even download
them. They are in the graph that `go list -m all`, an SBOM or a dependency
dashboard reads, and `go mod download all` fetches about 910 MiB where the
build needs 133 MiB. Every figure, with the command behind it, is in
[`docs/dependency-surface.md`](docs/dependency-surface.md); moving the CLI to a
module of its own is proposed there — with what it would cost — in
[ADR-0024](docs/adr/0024-cli-en-modulo-propio.md), and not taken here.
