// The per-engine test suites are their own module. They start real
// PostgreSQL, MySQL, MariaDB, SQL Server and Oracle databases in containers,
// so they require the five engine drivers and five testcontainers modules —
// eleven modules that no application importing Quark ever links, and that
// used to sit in the library's go.mod because these tests shared it
// (ADR-0024). Never published: the local replace directives point every Quark
...
replace github.com/jcsvwinston/quark/drivers/oracle => ../../drivers/oracle
