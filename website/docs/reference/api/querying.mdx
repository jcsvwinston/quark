...
The methods that end a query and return data: single rows, lists, aggregates,
pagination, and streaming. The builder methods that shape the query before that
point are in [Query Builder](query-builder); writes are in [CRUD](crud).

For filters, joins, and the rest of the builder in narrative form, see the
[Query Builder guide](../../guides/querying).

| Symbol | Signature | Purpose |
| --- | --- | --- |
| `First` | `First() (T, error)` | First matching row, or `ErrNotFound`. |
| `Find` | `Find(id any) (T, error)` | One row by single-column primary key. |
| `List` | `List() ([]T, error)` | Scan all matching rows into `[]T`. |
| `Count` | `Count() (int64, error)` | Count matching rows. |
| `Sum` | `Sum(column string) (float64, error)` | Sum of a column over matching rows. |
| `Avg` | `Avg(column string) (float64, error)` | Average of a column. |
| `Min` | `Min(column string) (float64, error)` | Minimum of a column. |
| `Max` | `Max(column string) (float64, error)` | Maximum of a column. |
| `Paginate` | `Paginate(pageSize, page int) (*Page[T], error)` | Count + limited select into a `*Page[T]`. |
| `Cursor` | `Cursor() (*Cursor[T], error)` | Type-safe cursor over `sql.Rows`. |
| `Iter` | `Iter(fn func(T) error) error` | Stream rows through a callback. |
...
count, err := quark.For[User](ctx, client).
    Where("active", "=", true).
    Count()
// => count is the number of matching rows, e.g. 2; err is nil
```
...
For two paid orders of `100.5` and `49.5`, `Sum("amount")` returns `150`.
...
fmt.Println(len(page.Items)) // => rows in this page (≤ pageSize)
fmt.Println(page.Total)      // => total matching rows across all pages
fmt.Println(page.TotalPages) // => ceil(Total / pageSize)
```

With three matching rows and `Paginate(2, 0)`, `page.Items` has length 2,
`page.Total` is 3, and `page.TotalPages` is 2.
