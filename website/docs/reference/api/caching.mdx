...
a complex WHERE, `UpdateBatch`, raw `Exec`) emit only the table tag —
the historical fallback. **Composite-primary-key** models also fall back
to the table tag; a follow-up may add a stable encoding of composite
keys for per-row invalidation if demand surfaces.
