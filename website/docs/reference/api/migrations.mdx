...
For which path to use when — quick reflective setup versus reviewable versioned
migrations — see the [Migrations guide](../../guides/migrations).
...
| `migrate.NewMigrator` | `NewMigrator(client *quark.Client, opts ...MigratorOption) *Migrator` | Binds a migrator to a client. Options: `WithoutLock()`, `WithLockTimeout(d)`, `WithLockName(name)`, `WithLogger(l)`. |
| `Migration.UpTx` / `Migration.DownTx` | `func(ctx, tx *sql.Tx) error` | Transactional forms: migration and ledger row commit together where the engine rolls DDL back. |
...
### `migrate.NewMigrator(client *quark.Client, opts ...MigratorOption) *Migrator`

Creates a migrator bound to a client.

```go
migrator := migrate.NewMigrator(client)
migrator = migrate.NewMigrator(client, migrate.WithLockTimeout(time.Minute), migrate.WithLogger(logger))
```

The migrator stores applied migration IDs in `quark_migrations`. `Up` and
`Down` take the engine's cluster-wide migration lock by default (`quark:schema`,
30s timeout); `WithoutLock()` skips it, `WithLockName` and `WithLockTimeout`
tune it, and `WithLogger` routes the progress lines (the default is the
client's logger). SQLite has no distributed lock and proceeds without one.
