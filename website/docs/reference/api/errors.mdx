...
receive an `ON` clause — via `.OnRaw(clause)` — that does not match the minimal
identifier-only grammar enforced by `internal/guard.ValidateJoinOn`. The grammar
accepts identifier-to-identifier comparisons (`a.b = c.d`) joined by `AND`/`OR`
and the operators `=`, `!=`, `<>`, `<`, `<=`, `>`, `>=`. Literals, function calls,
subqueries, and parentheses are rejected. The structured `.On(left, op, right)`
builder (delivered in v0.4) is the preferred form; `.OnRaw` is the escape hatch and
