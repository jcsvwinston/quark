...
fires `BeforeUpdate`), so timestamp / default / derived-field hooks apply to
batched rows just like single writes. `After*` hooks do not fire for batch ops —
see [Hooks › Limitations](./hooks#limitations).
