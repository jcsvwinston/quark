...
- **PostgreSQL.** `lib/pq` covers everything except the inbound `LISTEN/NOTIFY`
  listener ([Events](../advanced/events)). That one feature needs
  `github.com/jackc/pgx/v5/stdlib` and `quark.New("pgx", dsn)`. If you expect to
  use it, start with pgx and skip the migration later.
