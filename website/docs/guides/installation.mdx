...
- **PostgreSQL.** Either `lib/pq` or `github.com/jackc/pgx/v5/stdlib` works, and
  both report errors the same way to Quark. The one feature `lib/pq` cannot
  serve is the inbound `LISTEN/NOTIFY` listener
  ([Events](../advanced/events)), which needs pgx and `quark.New("pgx", dsn)`.
  If you expect to use it, start with pgx and skip the migration later.
