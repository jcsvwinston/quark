...
Applying `Limit`/`Offset` to a set-op without an explicit `OrderBy` is
portable: MSSQL and Oracle need an `ORDER BY` for their `OFFSET/FETCH`
pagination, so Quark auto-injects a positional `ORDER BY 1` (a select-list
ordinal — the only form a compound-select accepts; a bare column would be
rejected with "ORDER BY items must appear in the select list"). The other
dialects paginate without it. Add your own `OrderBy` on a projected column
when you need a defined order.
...
`WithRecursive(name, sub)` marks the CTE as recursive. The CTE name is
...
promotes the entire prefix to the recursive form.

The `RECURSIVE` keyword is **dialect-specific**. PostgreSQL, MySQL,
MariaDB and SQLite require it (`WITH RECURSIVE ...`). Oracle and SQL
Server reject it — they infer recursion from the query structure — so
Quark emits a plain `WITH ...` there. Quark selects the right spelling
per dialect; you call `WithRecursive` the same way on every engine.

:::note
Oracle additionally requires a column-alias list on a *genuinely*
recursive CTE (`WITH name (col1, col2) AS (anchor UNION ALL recursive)`).
Quark's CTE body is a raw `Subquery`, so a true Oracle recursive query
must include that alias list in the subquery's own SQL.
:::
...
| `op` is the standard comparison operator whitelist. | `=`, `!=`, `<>`, `<`, `<=`, `>`, `>=`, `LIKE`, `NOT LIKE`, `BETWEEN`, `NOT BETWEEN`, `IN`, `NOT IN`, `IS NULL`, `IS NOT NULL`. |
...
¹ MariaDB has no `FOR SHARE` keyword (that is MySQL-8 syntax — MariaDB raises
`Error 1064`), so `ForShare()` emits the older `LOCK IN SHARE MODE`. That form
takes no modifiers, so combining `ForShare()` with `SkipLocked()` or `NoWait()`
returns [`ErrUnsupportedFeature`](/docs/reference/api/errors) on MariaDB — use
`ForUpdate()` for those. Quark auto-detects MariaDB vs MySQL from the server
version (see [Installation](/docs/guides/installation)), so this is picked
automatically on a `"mysql"` connection.
