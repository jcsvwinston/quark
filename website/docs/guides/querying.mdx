...
| Dialect | Shape |
| --- | --- |
| PostgreSQL | `("metadata")::jsonb->>'plan' = $1` |
| MySQL | `JSON_EXTRACT(\`metadata\`, '$.plan') = ?` |
| MariaDB | `JSON_VALUE(\`metadata\`, '$.plan') = ?` |
| SQLite | `JSON_EXTRACT("metadata", '$.plan') = ?` |
| SQL Server | `JSON_VALUE([metadata], '$.plan') = @p1` |
| Oracle | `JSON_VALUE("METADATA", '$.plan') = :1` |

Use simple path keys. If your engine-specific JSON query needs arrays, casts, or
special operators, move that query to `RawQuery`.
