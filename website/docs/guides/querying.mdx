...
Engine support: PostgreSQL and SQL Server are full; MySQL/MariaDB do only
`UNION`/`UNION ALL` (others return `ErrUnsupportedFeature` — rewrite as a JOIN);
SQLite and Oracle lack the `ALL` variants of `INTERSECT`/`EXCEPT`. Applying
