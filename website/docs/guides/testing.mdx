...
`quarktest` is the test kit: a ready SQLite client with cleanup, one-line
schema setup, and the transaction-per-test isolation pattern. Three calls,
...
[native row-level security](../advanced/row-level-native.mdx), gate the
