...
See [Limitations](#limitations) below for the reasoning.
...
If you need different ordering — say, run an event emission after
ALL inserts have committed — register it explicitly. The public
`Tx.OnCommit(fn)` API in F5-5 (next release) covers this case;
until it ships, place the cross-cutting work after the `Client.Tx`
call returns nil:
...
- **Batch and WHERE-based operations** (`CreateBatch`,
  `UpdateBatch`, `DeleteBatch`, `DeleteBy`) currently bypass hooks.
  Bulk paths skip the per-row dispatch by design; `DeleteBy` issues
  a single WHERE-based DELETE without loading the affected rows, so
  there is no entity instance to call the hook on. If you need
  hooks for those paths, load the rows first and loop through the
...
- **Savepoints inside `Tx.Tx(nestedFn)`** roll back the SQL but do
  ...
  first time because F5-4 makes it observable. Track in TASKS.md
  for F5-5 follow-up; until then, prefer flat `Client.Tx` callbacks
  without nested savepoint Tx() over hook-heavy code, or call
  `After*` work explicitly via the (upcoming) `Tx.OnCommit` API
