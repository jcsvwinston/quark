...
| Dialect | Status |
| --- | --- |
| PostgreSQL | ✅ supported (this release) |
| SQLite | ✅ supported (this release) |
| MySQL | ⚠️ returns `ErrUnsupportedFeature` — F3-2-mysql follow-up |
| MariaDB | ⚠️ returns `ErrUnsupportedFeature` — F3-2-mariadb follow-up |
| SQL Server | ⚠️ returns `ErrUnsupportedFeature` — F3-2-mssql follow-up |
| Oracle | ⚠️ returns `ErrUnsupportedFeature` — F3-2-oracle deferred (no CI) |

Schema surface currently covers **tables and columns only**. Indexes,
foreign keys, and check constraints land in subsequent F3-2-*
follow-ups once the column-only path is verified across the 4 CI
engines. Code that consumes `Schema` should treat unpopulated
