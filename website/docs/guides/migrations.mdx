...
- **MySQL, MariaDB, Oracle** — DDL implicitly commits on every
  statement, so wrapping is pointless. `ApplyPlan` falls through to
  the no-tx path; a mid-plan failure leaves the schema partially
  applied. The returned error carries the op index and the op's
  `String()` so you can identify where things broke and resume
  manually. The eventual F3-4-resumable follow-up adds a
  `quark_migration_state` checkpoint table to automate resumption
  on these engines.
