...
The Plan is **inert** — it doesn't apply itself (`Plan.Apply` lands
with F3-3-execute). The CLI plan command (F3-5) will be the typical
consumer; in user code you can use `IsEmpty()` as a "did anything
drift?" probe in health checks or CI gates.
