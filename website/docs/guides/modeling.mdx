...
`created_at` and `updated_at` maintain themselves: `Create` fills both when
they are zero, and `Update` refreshes `updated_at` on every write (UTC).
Fields must be `time.Time` mapped to those column names — the scaffolded
models already qualify. A value you set yourself before `Create` is kept, so
imports and backdating keep working; timestamp hooks written for older
versions remain correct (they run first and the stamping only touches
zero values on create).
