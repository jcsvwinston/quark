...
| `quark:"index"` / `quark:"index=name"` | Declares a secondary index `Migrate` creates and `PlanMigration` proposes when missing. |
| `quark:"check=expr"` / `db:"col,enum=a\|b"` | Declares a `CHECK` on the column — an expression verbatim, or the allowed values of an enum — emitted as `ck_<table>_<column>`. |
...
| `db:"column,precision=18,scale=4"` | `Migrate`, `Sync`, mappers | Turns a `float32`/`float64` (or `Nullable` of one) into a fixed-point `DECIMAL(p,s)` — `NUMBER(p,s)` on Oracle — and reaches a `TypeMapper` through `TypeOptions`. On any other Go type the hint is ignored and the model carries a tag warning. |
...
:::caution If you are coming from Nucleus's `pkg/model`
Both read a tag called `db`, and they do not agree on what is inside it.

| | Quark | Nucleus `pkg/model` |
| --- | --- | --- |
| what the tag holds | the **column name**, plus options | **directives** |
| separator | comma | semicolon |
| primary key | a separate `pk:"true"` tag | `db:"pk"` |
| column name | `db:"email"` | `db:"column:email"` |
| not null / unique | `quark:"not_null,unique"` | `db:"not null;unique"` |

A model written the other way used to produce a Quark column called literally
`column:email;unique;not null` — no error, no warning, and a table nobody
could query. Quark now refuses a `db` tag whose column name cannot be an
identifier, and the error says how to spell it here.

A tag whose column name happens to be a legal identifier but reads like a
directive — `db:"pk"`, `db:"readonly"` — only warns: a table really can have a
column called `pk`, so refusing it would be wrong.
:::
...
`Restore` clears `deleted_at` for a row you've loaded. Restoring a row that is
already live affects zero rows instead of writing anything, so a misuse can't
corrupt live data:
...
`Update` is a **partial** update: it skips zero-value fields, so a half-filled
struct can't blank out a column with `false`, `0`, or `""`. To write a zero value
on purpose, name the column with `UpdateMap`:
...
value without a re-read. The version column is automatically `NOT NULL`, and one
...
`created_at` and `updated_at` maintain themselves. `Create` fills both when they
are zero, and `Update` refreshes `updated_at` on every write, in UTC. The fields
must be `time.Time` mapped to exactly those column names; the scaffolded models
already qualify.

A value you set yourself before `Create` is kept, so imports and backdating keep
working. Timestamp hooks written for older versions also stay correct: they run
first, and the automatic stamping only touches values still at zero.
...
func (u *User) BeforeCreate(ctx context.Context) error {
    u.Slug = slugify(u.Name) // derived fields, validation, defaults…
    return nil
}
```
...
Beyond Go's primitives, Quark ships generic wrappers for the common cases. Each
implements `sql.Scanner` and `driver.Valuer`, so reads and writes use the
...
`Migrate` emits `T`'s column type for you. The older `*time.Time` and
`sql.NullString` idioms still work, but `Nullable[T]` avoids the pointer's heap
allocation and reads more clearly at the call site — `p.Bio.Valid` rather than a
nil check.
...
It is deliberately **not** a PostgreSQL-native `TEXT[]` wrapper: operators like
`@>` and `array_agg` won't fire on the JSON column behind it. For native arrays
with operators, drop to `pgx`/`pgtype` or `RawQuery`.

A nil `V` serializes to `[]`, not `null`. Wrap it in `Nullable[Array[T]]` when
you need to tell SQL NULL apart from "valid but empty".

### Raw slices and maps, ranges, IP addresses

A raw `[]string`, `[]int64` or `map[string]any` field is stored as it is: on
PostgreSQL a slice of a scalar kind becomes a native array column (`TEXT[]`,
`BIGINT[]`, `DOUBLE PRECISION[]`, `BOOLEAN[]`) and the value travels as the
array literal; on every other engine — and for maps and slices of structs
everywhere — the column is the dialect's JSON type and the value JSON text.
`Array[T]` stays the JSON-backed wrapper on every engine when you want the same
column shape on all six.

`quark.Range[T]` is a bounded interval — `Lower`, `Upper`, `Bounds` (`"[)"` by
default) and `Empty` — stored as `TSTZRANGE`, `INT8RANGE`, `INT4RANGE` or
`NUMRANGE` on PostgreSQL and as JSON elsewhere. A `net.IP` field is an `INET`
column on PostgreSQL and a text column elsewhere, bound and scanned as the
address you would type.

The operators PostgreSQL has for these types — `@>`, `<@`, `&&` for containment
and overlap, `>>`, `<<`, `>>=`, `<<=` for networks — are accepted by `Where` on
PostgreSQL and refused with `ErrUnsupportedFeature` naming the engine anywhere
else, before any SQL is built. Nothing is rewritten into an approximation. To
ask whether a range contains a **point**, pass the point as the singleton range
`Range[T]{Lower: p, Upper: p, Bounds: "[]"}`: through a driver placeholder
PostgreSQL cannot tell a bare value from a range literal on the right of `@>`.
...
quark.For[Event](ctx, client).Where("tags", "@>", []string{"go"}).List()   // PostgreSQL only
at := time.Now()
quark.For[Event](ctx, client).Where("window", "@>", quark.Range[time.Time]{Lower: at, Upper: at, Bounds: "[]"}).List() // PostgreSQL only
quark.For[Event](ctx, client).Where("source", "<<", "10.0.0.0/8").List()   // PostgreSQL only
```
...
A mapped **primary key** keeps its key: the mapper's type is taken verbatim and
`PRIMARY KEY` is appended unless the mapper wrote it itself. (No auto-increment
is inferred from a mapped type.)

You may not need the mapper for UUIDs at all: any 16-byte array — the shape of
`google/uuid.UUID` — gets `UUID` on PostgreSQL, `CHAR(36)` on MySQL and
MariaDB, `VARCHAR2(36)` on Oracle, `NCHAR(36)` on SQL Server and a declared
`UUID` on SQLite, with the value travelling in its text form through the type's
own `Valuer` / `Scanner`. SQL Server's `UNIQUEIDENTIFIER` is deliberately not
used: the driver scans it as sixteen bytes in the engine's own byte order,
which a `Scanner` expecting the RFC order reads as a different UUID.

The mapper only decides the DDL type. The round-trip still goes through
`database/sql`, so the type must implement `sql.Scanner` and `driver.Valuer`, or
be natively supported by the driver. A `time.Duration` mapper ships built in; it
emits `BIGINT`, or `NUMBER(19)` on Oracle.
...
Precedence is **column tag → client default → pass-through**.

The wire contract is always UTC. A zoned column is converted to UTC on the way to
the database and back to the configured location when scanned, so the tag
changes only how the field reads in Go — never what is stored. An invalid IANA
name fails fast at `RegisterModel` or `Migrate` with `ErrInvalidTimezone`, so a
typo breaks startup instead of some later query.
