...
| `default:"value"` | `Migrate` | Emits `DEFAULT value` as written; **bool fields are normalized per dialect** (see Default values below). |
...
func init() {
    quark.RegisterTypeMapper(reflect.TypeOf(uuid.UUID{}), func(d string, _ quark.TypeOptions) string {
        ...
        case "postgres":
            return "UUID"
        case "mssql":
            return "NVARCHAR(36)" // never UNIQUEIDENTIFIER — see warning below
        default:
            return "VARCHAR(36)"
        }
    })
...
:::warning Don't map `uuid.UUID` to SQL Server's native `UNIQUEIDENTIFIER`
The example above deliberately uses `VARCHAR(36)` (textual) on every engine
except PostgreSQL. On SQL Server, **do not** return `UNIQUEIDENTIFIER`: a
round-trip silently changes the value. SQL Server stores the first three
groups of a GUID little-endian, while `github.com/google/uuid` (RFC&nbsp;4122)
is big-endian, so `go-mssqldb` hands back the bytes swapped — e.g. writing
`6a4c38e2-218a-4d93-…` reads back `e2384c6a-8a21-934d-…` (groups 1–3
reversed). The corruption is silent: no error, just a different UUID.

Stick with `VARCHAR(36)` / `NVARCHAR(36)` on SQL Server (it round-trips
cleanly and matches the other engines). Quark's automatic migrator already
maps a string-typed UUID primary key to `NVARCHAR(36)` on SQL Server for this
reason. If you genuinely need the native column, store the value as the
driver's own `mssql.UniqueIdentifier` type (which handles the byte-order)
...
expects — with one exception: **boolean defaults are normalized per dialect.**
A `bool` field whose default is `1`, `0`, `true`, or `false` (case-insensitive)
is rewritten to the literal each engine accepts — `TRUE`/`FALSE` on PostgreSQL,
`1`/`0` on the others — so the same tag is portable across all six engines.
There is otherwise no bool-default literal that works everywhere: PostgreSQL's
`BOOLEAN` rejects `1`, while MSSQL's `BIT` and Oracle's `NUMBER(1)` reject `TRUE`.

```go
Status    string    `db:"status" default:"'draft'"`
CreatedAt time.Time `db:"created_at" default:"CURRENT_TIMESTAMP"`
Active    bool      `db:"active" default:"1"`   // portable: normalized to TRUE / 1 per dialect
```
...
- **`quark.WithDefaultTZ(loc)`** — a Client-wide fallback for every
  `time.Time` column that doesn't carry its own tag.
...
[`ErrInvalidTimezone`](/docs/reference/api/errors), so a typo breaks the
app at startup rather than on the first query that touches the column.

A column with neither a tag nor a client default is untouched — the
feature is fully opt-in and adds nothing for callers that don't use it.
See [ADR-0010](https://github.com/jcsvwinston/quark/blob/main/docs/adr/0010-per-column-timezone-override.md)
