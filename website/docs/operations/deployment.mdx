...
`CGO_ENABLED=0 go build` works — the default of `scratch`/`distroless`
Dockerfiles and of every cross-compile (`GOOS=linux GOARCH=arm64 …`). The
only cgo-dependent piece, the `mattn/go-sqlite3` error classifier, sits
behind a build tag; pure-Go builds classify SQLite errors through
`modernc.org/sqlite` (the driver the [installation guide](../guides/installation)
recommends) with the same numeric codes. If you use SQLite in a static
binary, use the `modernc.org/sqlite` driver; every other engine's driver is
already pure Go except `mattn/go-sqlite3` itself.
...
— your `main` imports your migrations package and calls `commands.Main()`
(prints errors to stderr, exits non-zero on failure) —
