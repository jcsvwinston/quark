...
On PostgreSQL, a future `RowLevelSecurityNative` strategy (Fase 5,
F5-2) will delegate isolation to the database engine via `SET LOCAL
app.tenant_id` and `CREATE POLICY`. The two are mutually exclusive per
