...
are not encoded per-row yet, so they fall back to the table tag (like the
bulk methods below). Bulk and WHERE-based methods (`UpdateBatch`,
...
:::note Per-process by default; cross-instance is opt-in
The three protections below (singleflight, jitter, XFetch) are **per-process**:
each replica collapses its own concurrent callers, but N replicas can still each
compute the same hot key once. To coordinate across replicas, opt into
[`WithCacheCrossInstance()`](#cross-instance-coordination-opt-in) — a distributed
lock so a hot-key miss recomputes once across the fleet (ADR-0020).
:::
...
client, _ := quark.New("pgx", dsn,
    ...
    quark.WithCacheCrossInstance(),     // coordinate recompute across replicas
)
```

### Cross-instance coordination (opt-in)

`WithCacheCrossInstance()` (off by default) adds **deterministic** cross-replica
stampede protection on top of the per-process singleflight (ADR-0020). On a
hot-key miss, replicas race for a per-key distributed lock: the winner recomputes
and publishes the value; the losers wait briefly and re-read it instead of all
hitting the database. It takes effect only when the installed `CacheStore`
implements the optional `CacheLocker` capability — the bundled `redis.Store` does
(via `SET … NX` + a token-checked release); `memory.Store` provides the
single-process equivalent. A store without `CacheLocker` falls back to
singleflight + XFetch unchanged. Cost: one lock round-trip per hot-key miss, so
opt in when you run multiple instances against a shared cache and a recompute is
expensive.
...
| `WithCacheCrossInstance()` | off | Opt-in cross-replica recompute serialisation (ADR-0020). No effect unless the store implements `CacheLocker` (see [above](#cross-instance-coordination-opt-in)). |

(Jitter/XFetch/singleflight are per-process; cross-instance coordination is the
opt-in above.)
...
`pg_notify`. The **inbound** side ships too as of v1.1.0:
`ListenerFactory.CreateListener` returns a working `EventListener` on PostgreSQL
(other dialects return `ErrDialectNotSupported`) — see the
[Event Bus](events) guide for the listener API and ADR-0019. Delivery is
fire-and-forget: notifications emitted while the listener is down are lost (no
durable replay). For **outbound** CRUD lifecycle events, see the same guide
(`Client.UseEventBus`).
