...
| `TenantRouter.Tx` | `Tx(ctx, fn func(*Tx) error) error` | Opens a transaction confined to the resolved tenant; `ForTx` inside `fn` inherits the confinement. |
| `ErrTenantMismatch` | `error` sentinel | A query built inside a transaction resolved a different tenant than the transaction was opened for. |
...
The router exposes four methods. `ResolveTenant(ctx)` returns the tenant ID the
resolver extracted, `GetClient(ctx)` returns the `*Client` that ID maps to,
`ActiveTenants()` lists the tenant connections currently cached, and `Tx(ctx, fn)`
opens a transaction confined to the resolved tenant (see
[Transactions](../../advanced/multi-tenant#transactions)). Normal query code
needs only the last one — pass the router itself to `For`, and the `*Tx` it
hands you to `ForTx`.
