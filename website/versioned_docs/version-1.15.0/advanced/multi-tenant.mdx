...
| Engine | Row-level isolation | Why |
| --- | --- | --- |
| PostgreSQL | `RowLevelSecurityNative` (engine-enforced) or `RowLevelSecurityClient` | `set_config(..., true)` is transaction-scoped and `CREATE POLICY` filters server-side. |
| MySQL, MariaDB, SQLite | `RowLevelSecurityClient` | No engine-side row security. |
| SQL Server, Oracle | `RowLevelSecurityClient` | Both have engine-side row security (`CREATE SECURITY POLICY` over `SESSION_CONTEXT`; `DBMS_RLS` over a session context), but the context is **session**-scoped: set inside a transaction it survives the commit on the pooled connection, and the next tenant to borrow that connection would inherit it. Quark does not offer a strategy it cannot make safe on a pool. |

A `RowLevelSecurityNative` router over any engine but PostgreSQL refuses on all
three of its doors — `For`, `Tx` and `GetClient` — with `ErrUnsupportedFeature`.
`SchemaPerTenant` and `DatabasePerTenant` work on every engine.

## Transactions

A transaction opened for a tenant is confined to it, and so is every query built
inside it:

```go
err := router.Tx(acme, func(tx *quark.Tx) error {
    docs, err := quark.ForTx[Document](acme, tx).List() // acme's rows only
    if err != nil {
        return err
    }
    return quark.ForTx[Document](acme, tx).Create(&Document{Title: "Invoice 1002"})
})
```

`router.Tx` resolves the tenant from the context, opens the transaction on the
right client — the tenant's own pool, or the shared one — and stamps it.
`ForTx` then applies the same confinement `For` applies: the schema prefix, the
injected predicate, or, under [Native RLS](row-level-native), the session
variable the transaction already set. Relations loaded with `Preload` inside it
are qualified the same way.

**The transaction fixes the tenant.** Under Native RLS the engine is already
filtering by the tenant the transaction set; under database-per-tenant the pool
was chosen for it. So a query context inside the transaction that resolves a
*different* tenant fails with `quark.ErrTenantMismatch` rather than reading or
writing across tenants, and a context that resolves no tenant inherits the
transaction's — `quark.ForTx[T](context.Background(), tx)` is fine.

`router.GetClient(ctx)` followed by `client.Tx(ctx, ...)` on the shared base
client is the same door: when the context carries a tenant, the transaction is
confined exactly as `router.Tx` confines it. With no tenant in the context the
transaction stays what the base client always gave — one on the shared pool,
unconfined — which is how migrations and provisioning run.

Earlier releases handed back a bare transaction from `router.Tx` for every
strategy except Native RLS, so a query built inside it read every tenant's rows
and wrote to the default schema (QK-26). If you worked around that by adding the
predicate or the schema by hand inside `router.Tx`, the workaround is now
