...
a `Table` in a desired `Schema` carries its `ForeignKeys`, and `ApplyPlan` emits
them inline with the `CREATE TABLE` on every engine (see below).
...
A model can declare a single-column index in its tags, and both `Migrate` and
`PlanMigration` read the declaration:

```go
type User struct {
    ID    int64  `db:"id" pk:"true"`
    Email string `db:"email" quark:"unique"`          // a UNIQUE column constraint
    Slug  string `db:"slug" quark:"index"`            // idx_users_slug
    Org   string `db:"org" quark:"index=ix_users_org"` // a name of your choosing
}
```

`Migrate` creates the index after the table; `PlanMigration` proposes it when it
is missing and treats a live index of the same shape under another name — the
one the engine gives a `unique` column — as already there.
...
and the indexes they declare with `quark:"index"`. Live indexes, foreign keys
and check constraints the tags say nothing about are carried over — so a plan
never proposes dropping an object your models simply don't describe. A table
created by `ApplyPlan` is interchangeable with one from `Migrate`: same primary
key, same auto-increment shape.

A `Schema` you build by hand, or read from a document, can carry more than the
tags can: `Indexes`, `ForeignKeys` and `Checks` on each `Table`. `ApplyPlan`
creates the whole table — foreign keys and checks inline with the `CREATE
TABLE`, which is the one form SQLite accepts, and each index as its own
`CREATE INDEX` — and `Diff` orders new tables so a referenced table exists
before the table that references it. Re-diffing the desired schema against the
result proposes nothing: the loop converges. A foreign key is matched by its
columns and target, not by its name (SQLite keeps none), and an unspecified
`ON DELETE` / `ON UPDATE` is the engine default, `NO ACTION`.
...
- **Renaming** a column through `OpAlterColumn` returns `ErrUnsupportedFeature`:
  a rename is `Sync`'s `quark:"rename:old"` tag or a versioned migration.
- On **SQLite**, a table whose `CREATE TABLE` carries a `CHECK` in a shape the
  rebuild cannot read back (anything but `CONSTRAINT name CHECK (expr)` on its
  own line) returns `ErrUnsupportedFeature` rather than rebuilding without it.

`OpAlterColumn` covers the four facets a column can change — **type, nullable,
default and primary key** — on every engine. PostgreSQL alters each facet on
its own; MySQL and MariaDB restate the column with one `MODIFY`; SQL Server
alters the column and manages its named default and key constraints; Oracle
emits one `MODIFY` with exactly what changed. SQLite has no `ALTER COLUMN` and
no `ADD` or `DROP CONSTRAINT` at all, so there every column, foreign-key and
check change goes through the table rebuild its manual documents — create the
changed table under a temporary name, copy the rows, drop, rename, recreate the
indexes and triggers — inside the plan's transaction, so a failure (a `NOT
NULL` over rows that hold `NULL`, say) leaves the table as it was.
...
`Plan.Down()` returns the plan that undoes one: every operation inverted, in
reverse order.

```go
plan, _ := client.PlanMigration(ctx, &User{})

down, err := plan.Down()   // capture it BEFORE applying
if err != nil {
    return err
}
if err := client.ApplyPlan(ctx, plan); err != nil {
...
and then indexes it has to drop the index first, or the drop refers to
something that is already gone.

**Capture `Down` before you apply the plan.** Nothing stops you calling it
afterwards — a `Plan` is a value — but a plan built from a schema you have
already changed describes a different starting point.

:::caution Some operations cannot be reversed from a plan
`DROP TABLE` records the table's **name**, not its columns, keys and indexes.
`DROP COLUMN` records a name, not a type, nullability or default. There is
nothing in the plan to rebuild them from, so `Down` returns
`ErrIrreversibleOperation` naming the operation instead of emitting a partial
rollback.

That refusal is the point: a rollback that silently restores a table without
its indexes reports success and leaves the schema subtly different. When you
need to reverse a drop, write the `up`/`down` pair as a versioned migration,
where both directions are yours to state.
:::
