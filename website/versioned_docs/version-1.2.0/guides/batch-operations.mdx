...
| `UpsertBatch` | Dialect-specific bulk upsert | One statement (N `MERGE` on Oracle) |
...
engine's bind-parameter ceiling (SQL Server caps a statement at ~2100 parameters),
so you can pass tens of thousands of rows without a "too many parameters" error.
