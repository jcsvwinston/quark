...
From then on, each write inserts an audit row **on the same connection and
transaction as the write itself**, so the trail is atomic with the data.
...
That is a deliberately **stronger** guarantee than the [Event Bus](events),
whose post-commit emit can be lost on a crash. Losing an event is tolerable;
