...
`go install github.com/jcsvwinston/quark/cmd/quark@latest`, or a signed prebuilt
archive from the [releases page](https://github.com/jcsvwinston/quark/releases)
(see [Operational Workflows](cli) for what it does and
[Verifying a release](../operations/verifying-releases) for how to check an
archive) — but you don't need it to use the library.

The CLI is a separate Go module with its own version numbers: `@latest`
resolves a `cmd/quark/vX.Y.Z` tag, not a library one, so `@v1.13.0` and its
kind do not select it. That is a packaging boundary and not a compatibility
one — `quark version` prints the CLI's version and the Quark release inside
it, and the two travel together in every release.
...
workers, CLIs, or any layer that can pass a `context.Context`. For the
handler pattern in net/http, chi, Echo, Gin and Nucleus, see
