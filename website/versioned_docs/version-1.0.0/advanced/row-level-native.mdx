...
rejects rows the tenant may not see, even through raw SQL. The client-side
strategy only adds a predicate to the queries Quark builds.
...
- **No `LISTEN/NOTIFY` integration.** Inbound channel listening stays
  out of scope. Outbound CRUD events ship via
