...
By the end of this page you'll have a small but complete users service: a model,
its table, and every basic operation against it — insert, read, list, update,
delete. It takes about ten minutes.

The examples run on SQLite, so there is nothing to install. The same code runs
on PostgreSQL, MySQL, MariaDB, SQL Server, and Oracle by changing one string.
...
The default logger prints one informational line when the client starts
...
(more on that below). Pass [`quark.WithLogger`](/docs/reference/configuration)
...
[Migrations](migrations) — but you don't need either yet.
