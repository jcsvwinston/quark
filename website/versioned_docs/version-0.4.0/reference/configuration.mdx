...
Quark configuration is per client. `quark.New(db, ...options)` does not modify
global ORM state.
...
`quark.New` validates that `db` is non-nil and pings it with a five-second
startup timeout.
...
| Option | Purpose |
| --- | --- |
| `WithDialect(d Dialect)` | Sets SQL dialect explicitly. |
...
Auto-detection exists, but explicit dialects are easier to review and test.
