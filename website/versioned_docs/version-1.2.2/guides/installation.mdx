...
SQLite is the easiest place to start — no external service. For PostgreSQL,
`lib/pq` covers everything except the inbound `LISTEN/NOTIFY` listener
([Events](../advanced/events)) — that one feature requires
`github.com/jackc/pgx/v5/stdlib` (`quark.New("pgx", dsn)`); if you plan to use
it, start with pgx directly. For MySQL/MariaDB,
