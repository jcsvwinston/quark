...
This page works through them in that order, after one build-time note that
catches people out when they containerize.
...
None of the driver modules needs cgo: `drivers/sqlite` links the pure-Go
`modernc.org/sqlite`, and every other engine's driver is pure Go as well. The
one cgo-dependent SQLite driver, `mattn/go-sqlite3`, is not what the
[installation guide](../guides/installation) recommends; if you use it anyway,
register your own classifier for it (it reports the same numeric codes as
modernc) and keep the build tag in mind.

So the only rule is: if you use SQLite in a static binary, import
`github.com/jcsvwinston/quark/drivers/sqlite`.
...
- `WithLimits` replaces the **entire** `Limits` struct. Always start from
  `quark.DefaultLimits()` and override fields. A partial literal has its numeric
  zeros filled in from the defaults, but its **booleans do not** — so
  `SafeMigrations` silently lands on `false`, which permits a destructive `Sync`.
  See [Zero-value semantics](../reference/configuration#zero-value-semantics).
...
migration files register themselves in an `init()`, so a `go install`ed `quark`
...
[`quarkmigrate`](../guides/migrations#a-ci-drift-gate-with-quarkmigrate) package
