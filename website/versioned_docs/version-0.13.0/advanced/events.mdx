...
the helper `Notify`) is a separate, unrelated mechanism that remains
out of scope for Fase 5 — it needs a dedicated connection outside the
pool. `ListenerFactory.CreateListener` returns `ErrDialectNotSupported`.
