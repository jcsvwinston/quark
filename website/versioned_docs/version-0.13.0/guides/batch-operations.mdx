...
|---|---|---|
| `CreateBatch` | Single `INSERT … VALUES (…), (…)` except Oracle identity fallback | Single statement for most dialects |
...
Inserts multiple rows in a single `INSERT` statement. When the dialect supports
`RETURNING`, primary keys are written back to each entity pointer.
