...
| Oracle | `DBMS_LOCK.ALLOCATE_UNIQUE` + `REQUEST(X_MODE, release_on_commit => FALSE)` — session-scoped, survives the implicit commits of DDL. Result code `1` mapped to `ErrLockTimeout`. **Requires `GRANT EXECUTE ON DBMS_LOCK TO <user>`** (not granted to schema users by default). |
...
Multi-tenant deployments where each tenant Client
