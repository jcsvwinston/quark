...
> ⁵ Quark wraps the cache backing with `stampedeStore` (singleflight + ±jitter + Vattani XFetch), so a hot key never produces a database stampede on miss. No competitor in this matrix ships stampede protection out of the box.
