...
historical behaviour; 0 or negative also disables). Between attempts
the runner sleeps with exponential backoff + ±50% jitter
(10ms → 20ms → 40ms → … capped at 1s). A cancelled context aborts the
backoff and returns the context error.
...
Disabled by default: callers explicitly opt in to retry semantics —
the at-most-once-per-call contract that has shipped since v0.1 stays
the default. SQLite is single-writer and never raises a true deadlock,
