...
| `WithDefaultTZ(loc *time.Location)` | Fallback timezone for `time.Time` columns without a `quark:"tz=..."` tag. Values are stored as UTC; `loc` only affects how scanned values read in Go (ADR-0010). |
| `WithCacheJitter(pct float64)` | ±jitter factor applied to cache TTLs (anti-stampede, ADR-0011). Default `0.1`; clamped to `[0, 1]`. Needs `WithCacheStore`. |
| `WithCacheXFetchBeta(beta float64)` | XFetch probabilistic early-refresh parameter (ADR-0011). Default `1.0`; `0` disables XFetch (singleflight + jitter stay on). Needs `WithCacheStore`. |
| `WithCacheCrossInstance()` | Cross-instance cache-stampede coordination via a `CacheLocker` store (ADR-0020). Off by default; falls back to in-process singleflight when the store can't lock. |
| `WithReplicas(dsns ...string)` | Read-replica DSNs (ADR-0015): reads route to replicas, writes stay on the primary; transient replica failures fail over to the primary. |
