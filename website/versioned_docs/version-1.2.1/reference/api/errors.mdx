...
is the preferred form; `.OnRaw` is the escape hatch for clauses
...
`ErrEventEmitFailed`, `ErrListenerClosed`, and `ErrNoSubscription` cover the
event subsystem. `ErrEventEmitFailed` wraps an `EventBus.Publish` failure that
happens *after* the transaction commits — the commit stands, but the post-commit
emit could not be delivered. `ErrListenerClosed` and `ErrNoSubscription` come
from the inbound PostgreSQL `LISTEN/NOTIFY` listener: calling `Listen` after the
listener is closed, or `Receive` before any channel has been subscribed. See
[Events](/docs/advanced/events) for the bus and listener contracts.
