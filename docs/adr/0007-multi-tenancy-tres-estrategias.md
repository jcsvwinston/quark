...
- `RowLevelSecurity` (renombrar a `RowLevelSecurityClient` en Fase 5, ver ADR 0003): inyecta `WHERE tenant_id = ?` en cada query del builder.

Una aplicación puede usar `DatabasePerTenant` para datos críticos (PCI, médicos) y `RowLevelSecurity` para datos no sensibles, en el mismo proceso.
...
- `RowLevelSecurity` actual es client-side (ver ADR 0003) — riesgo de que usuarios crean que es RLS de motor.
