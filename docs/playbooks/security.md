...
- **`ValidateOperator(op)`**: whitelist (`=`, `!=`, `<>`, `<`, `<=`, `>`, `>=`, `LIKE`, `NOT LIKE`, `IN`, `NOT IN`, `IS`, `IS NOT`, `IS NULL`, `IS NOT NULL`, `BETWEEN`, `NOT BETWEEN`). No `ILIKE` — PostgreSQL-only and not implemented; case-insensitive matching is the caller's via `LOWER(...)`.
