...
- **`task_3be1f54e` — Finding C: `CreateBatch` no rellena los IDs generados en Oracle.** La medición S7 (2026-06-15) destapó que `CreateBatch(entities)` inserta las filas pero deja `entity.ID == 0` en **Oracle** (el `Create` single sí los rellena; los otros 5 motores también). Probado con un batch de 4 cuentas contra el `quark-oracle` persistente: `Count()==4` (las filas existen) pero `accs[i].ID==0` para todas; el mismo batch en MariaDB da 1,2,3,4. El `WhereBetween("id", accs[0].ID, accs[3].ID)` del exerciser `builder-advanced` devuelve 0 en Oracle por esto. **Bug de core, P1** (dato silencioso erróneo: el código aguas abajo usa `entity.ID==0`). Probable: `CreateBatch` (`query_crud.go:1646`) + el path batch del dialecto Oracle (necesita `RETURNING … INTO` por fila). Fix con regresión en la SharedSuite (6 motores). Chip abierto + trazado aquí por si no persiste.
