...
- [~] **S7 · CI** — job 6-motores (patrón `integration`; Oracle docker-run); gate estricto bloqueante. **Medido 2026-06-15** + **re-medido 2026-06-17** (ver § "S7 — medición de cobertura" abajo). S7 decompone en diag✅/fix-harness(en curso)/coverage/ci. **S7-fix-harness:** los 4 fallos harness-side del 2026-06-15 cerrados (mysql zero-time en Project/Membership, mssql migrate-into-schema, BD dedicada mysql/mariadb/mssql, oracle conns≤4) — mariadb pasó a verde funcional. mysql/mssql/oracle avanzaron y destaparon **3 bugs de core nuevos** (Findings F/G/H abajo), que van como `fix:` propios antes de S7-coverage/ci.
