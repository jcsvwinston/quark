...
- [x] **S7 · CI** — job 6-motores (patrón `integration`; Oracle docker-run); gate estricto bloqueante. S7 decompone en diag✅/fix-harness✅/coverage/ci. **S7-fix-harness CERRADO** (2026-06-17): los 4 fallos harness-side (PR #200) + los **3 bugs de core que destaparon — Findings F/G/H — RESUELTOS** (PRs #201/#203/#204, mergeados; cada uno con regresión SharedSuite 6-motores + docs). Verde funcional: sqlite/postgres/mariadb plenos; mysql/mssql avanzaron 82→130 / 83→117 pasando sus fallos originales. El run profundo destapó 2 findings de core nuevos (I/J abajo) + un harness-gap de re-entrancia; **I/J RESUELTOS** (PR #206, mergeado 2026-06-17 — Upsert/UpsertBatch hooks + set-op Limit ORDER BY, regresiones SharedSuite 6-motores). **Post-I/J run HECHO (2026-06-18):** `builder-advanced` funcionalmente limpio en los 6 (sqlite 167 / pg·mysql·mariadb·mssql 168; Oracle vía SharedSuite F + el run previo a 105) — **F/G/H/I/J cerraron todos los bugs de core, sin findings nuevos**. El único gap era de harness (NoWait no tolerado en MSSQL, capacidad-desigual) → cerrado en **PR #213**. **S7-coverage + S7-ci CERRADOS (2026-06-18):** denominador allowlist (#215: `gen-allowlist` + `allowlist.json` — dialecto/type/const/var, preserva `RowLevelSecurity`), exercisers de la superficie callable + **gate de aceptación 6-motores bloqueante** (#216: job "Superapp acceptance (6 engines)" corre `go run ./examples/superapp -engines=all -gate=strict`, Oracle docker-run; + step "superapp manifest freshness (S7-coverage)" en Lint que regenera y `git diff`). **S7 COMPLETO**; quedan abiertos S8 (snapshots golden) y S9-full (inventario cobra + golden + matriz cross-engine).
...
Ejecutado (todo CERRADO):
- ~~**PR #1 — `allowlist.json` (denominador)**~~ ✅ **#215**: `gen-allowlist` deriva del manifiesto (razón por categoría — dialecto / type / const / var; preserva `RowLevelSecurity`); el gate pasó a medir la superficie **callable**.
- ~~**PR #2+ — exercisers de la superficie callable**~~ ✅ **#216**: exercisers para los callable del API público (TypedColumn/NewTypedColumn, métodos de Client/Tx, funcs de subpaquetes cache/quarktenant/quarkmigrate/otel/migrate, Cursor/Array, etc.); gate estricto en verde.
- ~~**S7-ci — job bloqueante 6-motores**~~ ✅ **#216**: job "Superapp acceptance (6 engines)" (`-engines=all -gate=strict`, Oracle docker-run) + freshness en Lint; ambos bloqueantes en CI.
