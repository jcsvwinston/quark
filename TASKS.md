...
- [~] **S5 · `exercise/`** — *En curso.* Harness `suite.go` (`Run()`: recorder por motor → exercisers → cobertura `control.Invoked`; reusa `engine.Run` para lifecycle+anti-fugas) + helpers de key (`QM`/`CM`/`QF`, casan exacto con `apisurface.json`). **Entregados:** `crud` (Create/First/Count/Update/Delete-soft/List; asserts de round-trip + exclusión soft-delete) y `tx` (commit multi-entidad atómico + rollback verificado). Verdes en SQLite in-process **y Postgres docker-run real** (15 símbolos / 14 statements / sin fugas, idénticos en ambos). **Faltan:** builder (CTE/window/setops/locking) → relations (confirmar m2m/poly vs `relations.mdx`) → cache (query-count) → tenant → migrate (round-trip `Plan` vacío) → security (attack suite SQLGuard) → ha (replicas/sharding/deadlock) → observability (OTel in-mem), + el **oráculo de paridad** cross-engine (comparar resultados de `fn` entre motores, no sólo asserts por-motor).
