...
- [~] **S7 · CI** — job 6-motores (patrón `integration`; Oracle docker-run); gate estricto bloqueante. S7 decompone en diag✅/fix-harness✅/coverage/ci. **S7-fix-harness CERRADO** (2026-06-17): los 4 fallos harness-side (PR #200) + los **3 bugs de core que destaparon — Findings F/G/H — RESUELTOS** (PRs #201/#203/#204, mergeados; cada uno con regresión SharedSuite 6-motores + docs). Verde funcional: sqlite/postgres/mariadb plenos; mysql/mssql avanzaron 82→130 / 83→117 pasando sus fallos originales. **El run profundo destapó 2 findings de core NUEVOS (I/J abajo) + un harness-gap de re-entrancia** — ninguno es regresión; decisión de alcance pendiente. **Siguiente:** S7-coverage (allowlist dialectos+tipos/consts/vars + exercisers de los ~192 callable) y S7-ci (job bloqueante) — ahora con la fila funcional mayormente verde. I/J pueden ir antes si se decide.
...
- **Finding I (NUEVO, ABIERTO): `Upsert` (single) no ejecuta `BeforeCreate`.** Destapado por el run limpio post-F/G/H (mysql alcanzó `builder-advanced/Upsert` por primera vez). `Upsert` (`query_crud.go:1458`) valida pero no invoca `BeforeCreate`, así que la fila insertada queda con timestamps cero → MySQL strict rechaza `'0000-00-00'` en `created_at`. Hermano de Finding H pero en `Upsert`/`UpsertBatch`, **con la ambigüedad que H dejó abierta para upsert**: una operación insert-OR-update no sabe en build-time qué hook disparar (`BeforeCreate` vs `BeforeUpdate`). **Requiere decisión del owner** (igual que H la requirió para batch) antes de fix `fix:` + regresión 6 motores.

- **Finding J (NUEVO, ABIERTO): MSSQL `Union`+`Limit` genera un ORDER BY inválido bajo UNION.** Destapado por el run limpio post-F/G/H (mssql alcanzó `builder-advanced/Union`). `admins2.Union(members).Limit(10).List()` en MSSQL → `OFFSET/FETCH` (la `LimitOffset` de MSSQL exige ORDER BY) y el ORDER BY que Quark inyecta no referencia columnas del select-list → `ORDER BY items must appear in the select list if the statement contains a UNION...`. Quark genera SQL inválido para MSSQL en Union+Limit (el caller escribió una query razonable). Bug de core (dialecto/builder); fix `fix:` (ORDER BY ordinal/select-list bajo UNION en MSSQL, o suprimir el ORDER BY sintético cuando hay setop) + regresión 6 motores.
