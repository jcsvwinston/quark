...
- **`task_20d5f912` — Finding A: ApplyPlan crea tablas sin PK/autoincrement.** El exerciser `migrate` (S5) destapó que `OpCreateTable` vía `ApplyPlan` emite el CREATE TABLE sin constraint de PK: `modelsToSchema` pasa `IsPK:false` (gap "F3-2-pk" ya anotado en el godoc de `migrate_plan.go`) y `applyCreateTable` (`migrate_execute.go:371`) renderiza sólo tipo/null/default. La tabla resultante rechaza INSERTs con id autogenerado (`NOT NULL constraint failed` en SQLite) y **diverge en silencio** de la que emite `Migrate` — el diff tampoco compara PK, así que el plan posterior sale vacío. El bug-bash F6 no lo vio porque sólo ejercitó `ApplyPlan` con OpAddColumn. Workaround del arnés: la tabla nueva se crea vía `Migrate`; `ApplyPlan` se ejercita en ops de columna y drop-table (comentario en `exercise/migrate.go` paso 2).

- **`task_b03f2155` — Finding B: PlanMigration propone drift falso (y parcialmente destructivo) sobre una BD recién migrada.** En el run PG del superapp, `PlanMigration(AllModels)` tras `Migrate` devuelve **14 ops** en vez del plan vacío documentado ("did anything drift?" check de CI) — 3 clases: (1) **`DROP TABLE project_tags`**: las join tables m2m que `createJoinTables` crea no entran en el desired de `modelsToSchema` → aplicar el plan **destruye la join table y sus datos** (también en SQLite); (2) **alters cosméticos de default**: PG devuelve `'member'::text`/`false` y `defaultsEqual` (`migrate_diff.go:529`) compara strings crudos → `OpAlterColumn` permanente en toda columna con default, y encima `ApplyPlan` no ejecuta alters de sólo-default (`ErrUnsupportedFeature`, "F3-3-execute-alter") → plan inaplicable; (3) **alias de tipo sin normalizar**: `timestamp without time zone` vs `TIMESTAMP` (`normalizeType` no lo colapsa) → op de sólo-tipo por cada columna `time.Time`. (2)–(3) son la refinación que el comentario de `migrate_plan.go` (PR #170) dejó "tracked separately"; (1) es nueva. Workaround del arnés: `filterKnownDrift` en `exercise/migrate.go` (quirúrgico: drop m2m conocido + predicado de equivalencia cosmética; todo lo demás sigue rompiendo el assert). Al cerrar cada clase, retirar su filtro y endurecer a `IsEmpty()`.
