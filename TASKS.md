...
- [ ] **S7 · CI** — job 6-motores (patrón `integration`; Oracle docker-run); gate estricto bloqueante. **Medido 2026-06-15** (ver § "S7 — medición de cobertura" abajo): el gate está rojo — 4 fallos funcionales (2 son bugs de core, Findings C/D) + gap estructural de denominador. S7 decompone en diag✅/fix-harness/coverage/ci; las core bugs van primero.
...
- **`task_da9dfae1` — Finding E: `CreateBatch` no invalida el table tag de caché L2 en dialectos RETURNING (PG/SQLite/MariaDB).** Destapado al revisar el fix de Finding C. El path RETURNING de `createBatchStmt` (`query_crud.go:~1786`) scanea los PKs vía `executeQueryPrimary` → `executeQueryOn`, que **no invalida nada** (a diferencia de `executeExec`, que dropea el table tag en writes). Resultado: tras un `CreateBatch`, las lecturas de lista cacheadas quedan stale en PG/SQLite/MariaDB (MySQL/MSSQL van por `executeExec` → OK; Oracle quedó OK tras Finding C). **Bug de core, P2** — clase BB-15 (hermano batch del fix #175, que arregló `Create` single). Fix: invalidar el table tag + row tags tras el scan, una vez por chunk; regresión cross-engine con cache store (List→CreateBatch→List). Chip abierto + trazado aquí.

- **`task_1a296d24` — Finding D: drift falso de `PlanMigration` en columnas JSON de MariaDB.** La medición S7 destapó que `PlanMigration` tras `Migrate` propone `ALTER … (longtext→JSON)` por cada columna JSON en **MariaDB** (que implementa JSON como alias de LONGTEXT: el catálogo reporta `longtext`, el modelo desea `JSON`). Misma clase que Finding B, caso JSON de MariaDB que `RoundTrip_RichFixture` no cubría. Root cause: `normalizeType` (`migrate_diff.go:723`) no tiene el caso `json`≡`longtext`. **Bug de core, P2** (drift cosmético/no destructivo, pero rompe el invariante "PlanMigration vacío tras Migrate" en MariaDB). OJO: MariaDB reporta `longtext` también para `longtext` real → fix **dialect-aware** (el introspector detecta el CHECK `json_valid()` y emite `json`), no un coerce ciego en `normalizeType`. Chip abierto + trazado aquí.
