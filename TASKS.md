...
- [~] **S5 · `exercise/`** — *En curso.* Harness `suite.go` (`Run()`: recorder por motor → exercisers → cobertura `control.Invoked`; reusa `engine.Run` para lifecycle+anti-fugas) + helpers de key (`QM`/`CM`/`QF`, casan exacto con `apisurface.json`). **Entregados:** `crud` (Create/First/Count/Update/Delete-soft/List), `tx` (commit multi-entidad + rollback), `builder`, `relations` (Preload belongs_to/has_many/m2m + persistencia de asociación; BB-5 FK nullable) (agregados Sum/Avg/Min/Max, Select+GroupBy+Having, WhereIn/Or, OrderBy/Offset/Distinct, Iter/Cursor, Find, Paginate). Verdes en SQLite in-process **y Postgres docker-run real** (32 símbolos / 55 statements / sin fugas, idénticos en ambos). El dominio restauró sus bool defaults (`default:"1"`/`"0"`) tras el fix del migrator (#170) — su migración en PG valida ese fix end-to-end; y el tag m2m se corrigió a `m2m:"project_tags:project_id:tag_id"`. **Gotchas de portabilidad cazados por el run en PG** (no son bugs de Quark, query mal escrito): `GroupBy` exige `Select` del col agrupado (si no, `SELECT *`+GROUP BY rompe en PG); comparar `bool` con bool, no con 0/1 (pgx estricto) — documentados en `builder.go`+HANDOFF. **Faltan:** builder-avanzado (CTE/window/setops/locking) → relations (confirmar m2m/poly vs `relations.mdx`) → cache (query-count) → tenant → migrate (round-trip `Plan` vacío) → security (attack suite SQLGuard) → ha (replicas/sharding/deadlock) → observability (OTel in-mem), + el **oráculo de paridad** cross-engine (comparar resultados de `fn` entre motores, no sólo asserts por-motor).
