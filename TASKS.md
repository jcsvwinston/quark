...
- [~] **S7 · CI** — job 6-motores (patrón `integration`; Oracle docker-run); gate estricto bloqueante. S7 decompone en diag✅/fix-harness✅/coverage/ci. **S7-fix-harness CERRADO** (2026-06-17): los 4 fallos harness-side (PR #200) + los **3 bugs de core que destaparon — Findings F/G/H — RESUELTOS** (PRs #201/#203/#204, mergeados; cada uno con regresión SharedSuite 6-motores + docs). Verde funcional: sqlite/postgres/mariadb plenos; mysql/mssql avanzaron 82→130 / 83→117 pasando sus fallos originales. El run profundo destapó 2 findings de core nuevos (I/J abajo) + un harness-gap de re-entrancia; **I/J RESUELTOS** (PR #206, mergeado 2026-06-17 — Upsert/UpsertBatch hooks + set-op Limit ORDER BY, regresiones SharedSuite 6-motores). **Siguiente:** un superapp run limpio post-I/J confirma si `builder-advanced` queda pleno en los 6 (o destapa más findings); luego S7-coverage (allowlist dialectos+tipos/consts/vars + exercisers de los ~192 callable) y S7-ci (job bloqueante). Pendiente menor: el harness-gap de re-entrancia (cache/crud no convergen sobre BD sucia — S8 polish, no afecta CI).
